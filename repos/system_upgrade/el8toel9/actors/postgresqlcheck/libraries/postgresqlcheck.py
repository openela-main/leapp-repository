...
report_server_inst_summary = (
    'PostgreSQL server component will be upgraded. Since OL-9 includes'
...
report_server_inst_hint = (
    ...
    ' and follow steps in the documentation section "Migrating to a OL 9 version of PostgreSQL"'
    ' after the upgrade.'
)

# Link URL for postgresql-server report
report_server_inst_link_url = 'https://docs.oracle.com/en/operating-systems/oracle-linux/9/install/'
...
def _report_server_installed():
    ...
    reporting.create_report([
        ...
        reporting.ExternalLink(title='Migrating to a OL 9 version of PostgreSQL',
...
def report_installed_packages(_context=api):
    ...
    Create the report if the postgresql-server rpm (OL signed) is installed.
    """
