...
DNF_PLUGIN_NAME = 'oracle_upgrade.py'
_DEDICATED_URL = 'https://docs.oracle.com/en/operating-systems/oracle-linux/9/leapp/leapp-TroubleshootingOracleLinuxUpgrades.html#issue-overlay"'
...
class _DnfPluginPathStr(str):
    ...
    def _feed(self):
        ...
        if major not in _DnfPluginPathStr._PATHS:
            raise KeyError('{} is not a supported target version of OL'.format(major))
...
def backup_debug_data(context):
    ...
    if config.is_debug():
        ...
        # debug_solver dnf config parameter in our ol-upgrade dnf plugin when LEAPP_DEBUG env var set to 1.
        try:
...
def _handle_transaction_err_msg_old(stage, xfs_info, err):
    ...
    if 'more space needed on the' in err.stderr and stage != 'upgrade':
        ...
        details = {'hint': "Please follow the instructions in the '{}' section of the article at: "
                           "link: https://docs.oracle.com/en/operating-systems/oracle-linux/9/leapp/leapp-TroubleshootingOracleLinuxUpgrades.html#issue-overlay"}
...
def perform_transaction_install(target_userspace_info, storage_info, used_repos, tasks, plugin_info, xfs_info):
    ...
    if get_target_major_version() == '8':
        ...
        # the target major version is OL 9+
    ...
    with _prepare_transaction(used_repos=used_repos,
        ...
        # on the host as the config-manager plugin is available since ol-8)
