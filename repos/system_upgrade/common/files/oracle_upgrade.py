...
class OracleUpgradeCommand(dnf.cli.Command):
    aliases = ('oracle-upgrade',)
    summary = ("Plugin for upgrading to the next Oracle Linux major release")

    def __init__(self, cli):
        super(OracleUpgradeCommand, self).__init__(cli)
        self.plugin_data = {}
...
class OracleUpgradePlugin(dnf.Plugin):
    name = 'oracle-upgrade'

    def __init__(self, base, cli):
        super(OracleUpgradePlugin, self).__init__(base, cli)
        if cli:
            cli.register_command(OracleUpgradeCommand)
