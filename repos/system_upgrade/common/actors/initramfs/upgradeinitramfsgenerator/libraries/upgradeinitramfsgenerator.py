...
def _get_target_kernel_version(context):
    ...
    kernel_version = None
    try:
        results_uek = context.call(['rpm', '-qa', 'kernel-uek-core'], split=True)
        if not results_uek:
            results_rhck = context.call(['rpm', '-qa', 'kernel-core'], split=True)
        if results_uek:
            versions = [ver.replace('kernel-uek-core-', '') for ver in results_uek['stdout']]
        else:
            versions = [ver.replace('kernel-core-', '') for ver in results_uek['stdout']]
