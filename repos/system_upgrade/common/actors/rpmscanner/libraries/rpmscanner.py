...
def get_package_repository_data():
    ...
    Note:
        There's no yum module for py3. The dnf module can be used only on OL 8+,
        on OL 7 there's a bug in dnf preventing us to do so:
    """
    if not no_yum:
...
def map_modular_rpms_to_modules():
    ...
    # empty on OL 7 because of no modules
    if not modules:
        return {}
