...
# types and attributes that where removed between OL 7 and 8
...
# types and attributes that where removed between OL 8 and 9
...
def get_selinux_modules():
    ...
    # udica templates
    template_list = []
    # list of rpms containing policy modules to be installed on OL 8
    install_rpms = []
    ...
    # This is necessary since container policy module is part of selinux-policy-targeted in OL 7 (but not in OL 8)
    try:
...
def get_selinux_customizations():
    ...
    which should be safe to re-apply on OL 8 system
