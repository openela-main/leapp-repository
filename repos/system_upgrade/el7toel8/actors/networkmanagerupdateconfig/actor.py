...
snippet_data = ("# Generated by leapp when upgrading from OL7 to OL8\n"
                "[main]\n"
                "dhcp=dhclient\n")
...
class NetworkManagerUpdateConfig(Actor):
    """
    Updates NetworkManager configuration for Oracle Linux 8.

    On Oracle Linux 7 NetworkManager uses the "dhclient" DHCP backend by default, while
    the default is "internal" on Oracle Linux 8. We want to keep "dhclient" enabled on
