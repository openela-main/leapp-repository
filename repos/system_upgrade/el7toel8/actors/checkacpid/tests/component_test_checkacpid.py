...
def test_actor_with_acpid_package(current_actor_context):
    with_acpid = [
        RPM(name='acpid', version='0.1', release='1.sm01', epoch='1', packager=RH_PACKAGER, arch='noarch',
...
def test_actor_without_acpid_package(current_actor_context):
    ...
    current_actor_context.feed(create_modulesfacts(installed_rpm=without_acpid))
    current_actor_context.run()
